...
load_dotenv()  # This loads variables from .env file in the current directory
import requests
from model.session_manager import init_session_db, create_session, validate_session_token, delete_session
...
# Resend API key (for 2FA OTP)
app.config['RESEND_API_KEY'] = 're_G7TuLJkG_RDZLeFmDsXC7Q9U8DmnaTqjC'  # from your .env
...
@app.before_request
def require_login():
    allowed_routes = ['login', 'register', 'static', 'new_account_buyer', 'new_account_developer', 
                  'forgot_pass', 'forgot_password', 'checkUser', 'create_buyer', 'create_developer', 
                  'index', 'two_factor_page', 'send_otp', 'verify_otp_route', 'payment_status_buyer']
    ...
    if not validate_session_token(session['username'], session.get('session_token', '')):
        # Invalid token – clear local session and redirect to login
        session.clear()
        return redirect(url_for('login'))
...
def login():
    ...
    if request.is_json:
        ...
        if user and user_active_check:
            # Store pending user info in session (not logged in yet)
            session['pending_2fa'] = True
            session['pending_username'] = user[0]
            session['pending_user_type'] = user[1]
            session['pending_store_region'] = user[2]

            # Get email address
            email_data = current_user_query(username)
            ...
            delete_old_otps(email)

            # Create and store OTP record
            otp_record = create_otp_record(email)
            save_otp_record(otp_record)

            # Always send to the test email address
            test_recipient = 'kansainoryu404@gmail.com'
            ...
            try:
                resend.Emails.send({
                    ...
                    "to": test_recipient,
                    "subject": "🔐 Your BashPo 2FA Code",
                    "html": f"""
                    <div style="font-family: Arial, sans-serif; max-width: 600px;">
                        <h2>Your One-Time Password (OTP)</h2>
                        <p>Hello {username},</p>
                        <p>Use the following code to complete your login:</p>
                        <div style="font-size: 32px; font-weight: bold; padding: 10px; background: #f4f4f4; display: inline-block;">
                            {otp_record['code']}
                        </div>
                        <p>This code expires in 5 minutes.</p>
                        <p>If you did not request this, ignore this email.</p>
                        <hr>
                        <p><small>This OTP was requested for account: {email}</small></p>
                    </div>
                    """
                })
                print(f"OTP sent to {test_recipient} for user {username} (actual email: {email})")
            except Exception as e:
                print(f"Resend error: {e}")
                return jsonify({"error": "Failed to send OTP email"}), 500

            # Return redirect to the 2FA page
            return jsonify({
                "success": True,
                "redirect_url": url_for('two_factor_page')
            }), 200

        elif user and not user_active_check:
            return jsonify({"error": "Account Terminated due to fraudulent activities"}), 401
        else:
...
def two_factor_page():
    ...
    if not session.get('pending_2fa'):
        return redirect(url_for('login'))
    # Pass the email (original user email) to the template for display
...
def send_otp():
    """Resend OTP code"""
    if not session.get('pending_2fa'):
        return jsonify({"success": False, "message": "No pending 2FA session"}), 400
    ...
    try:
        resend.Emails.send({
            "from": "onboarding@resend.dev",
            "to": test_recipient,
            "subject": "🔐 Your BashPo 2FA Code (Resend)",
            "html": f"""
            <div style="font-family: Arial, sans-serif;">
                <h2>New OTP Code</h2>
                <p>Your new verification code is:</p>
                <div style="font-size: 32px; font-weight: bold; padding: 10px; background: #f4f4f4;">
                    {otp_record['code']}
                </div>
                <p>Valid for 5 minutes.</p>
                <hr>
                <p><small>Requested for account: {email}</small></p>
            </div>
            """
        })
        return jsonify({"success": True, "message": "OTP resent", "expiresIn": 300})
    except Exception as e:
        print(e)
...
def verify_otp_route():
    if not session.get('pending_2fa'):
        return jsonify({"valid": False, "message": "No pending 2FA session"}), 400

    data = request.json
    otp = data.get('otp')
    if not otp:
        return jsonify({"valid": False, "message": "OTP required"}), 400

    username = session.get('pending_username')
    email_data = current_user_query(username)
    email = email_data[0] if email_data else None
    if not email:
        return jsonify({"valid": False, "message": "User email not found"}), 400

    valid, msg = verify_otp(email, otp)
    if valid:
        # OTP correct – finalise login
        session.pop('pending_2fa', None)
        session['username'] = session.pop('pending_username')
        session['user_type'] = session.pop('pending_user_type')
        session['store_region'] = session.pop('pending_store_region')
        session.permanent = True

        
        session_token = create_session(session['username'], expiry_minutes=30)
        session['session_token'] = session_token
        # Redirect based on user type
        if session['user_type'] == 'buyer':
            redirect_target = url_for('buyer_dashboard')
        elif session['user_type'] == 'developer':
            redirect_target = url_for('developer_dashboard')
        elif session['user_type'] == 'admin':
            redirect_target = url_for('admin_dashboard')
        else:
            redirect_target = url_for('index')
        return jsonify({"valid": True, "message": msg, "redirect": redirect_target})
    else:
...
def logout():
    if 'username' in session:
        delete_session(session['username'])
    session.clear()
    return redirect(url_for('login'))
...
def forgot_password():
    ...
    user_exists = check_user_query(username, None)   # from route_help
    if len(user_exists) == 0:
        return jsonify({"error": "Username not found."}), 404

    # Get the user's email (decrypted)
    email = get_user_email(username)   # from req_auth
    if not email:
        return jsonify({"error": "User email not found."}), 500

    # Update password with new hash
    forget_password_update_password(username, new_password)   # new function (see below)
...
def create_buyer():
   ...
   # Only check username uniqueness (email is encrypted and cannot be searched)
    user_check = check_user_query(username, None)
    if len(user_check) != 0:
...
def Pay_With_Card():
        ...
        'currency': "USD",
        'tran_id': trx_id,
        'success_url': f"http://127.0.0.1:1097/paymentBuyer/success-{buyer_username}",
...
def payment_status_buyer(response):
    # 1. Cart purchase success (username in URL)
    if response.startswith("success-"):
        buyer_username = response.split("-")[1]
        payment_success_card_purchase(buyer_username)   # modified function
        return render_template("buyer_game_card_status.html", response=True)
    
    # 2. Developer publishing fee
    if response.startswith("successDev"):
    ...
    # 3. Wallet top-up
    if response.startswith("successWallet"):
        purchase_success_card_wallet(response)   # already works without session
        return render_template("buyer_game_card_status.html", response="walletTrue")
    
    # 4. Wallet failures / cancels
    if response.startswith("failWallet") or response.startswith("cancelWallet"):
        return render_template("buyer_game_card_status.html", response="walletFalse")
    
    # 5. For all other cases (fail, cancel, etc.), require login
    if 'username' not in session:
        return redirect(url_for('login'))
    
    buyer_username = session['username']
    if response in ["failure", "cancel"]:
        return render_template("buyer_game_card_status.html", response=False)
...
@app.route('/get_review/<game_name>')
@login_required('buyer')
def get_review(game_name):
    buyer = session['username']
    with sqlite3.connect('bashpos_--definitely--_secured_database.db') as db:
        c = db.cursor()
        c.execute("SELECT encrypted_review, review_sig, rating FROM Reviews WHERE game_name = ? AND username = ?", (game_name, buyer))
        row = c.fetchone()
        if row:
            enc_rev, rev_sig, rating = row
            try:
                review_text = decrypt_admin_data(enc_rev, rev_sig)
                return jsonify({'exists': True, 'review': review_text, 'rating': rating})
            except:
...
def update_review():
    ...
    update_review_query(session['username'], game_name, rating, review)
...
def buyer_profile():
    buyer_username = session['username']
    balance,buyer_username,buyer_details,status,card_info,pending_requests,my_friends,wishlist_value,wishlist_user,cart_status,cart_value,owned_games=buyer_dashboard_query(buyer_username)
...
def Wallet2Credit():
    if request.method == 'POST':
        ...
        post_body = {
            ...
            'currency': "USD",
            'tran_id': trx_id,
            'success_url': f"http://127.0.0.1:1097/paymentBuyer/successWallet-{buyer_username}-{amount}",
            'fail_url': f"http://127.0.0.1:1097/paymentBuyer/failWallet-{buyer_username}",
            'cancel_url': f"http://127.0.0.1:1097/paymentBuyer/cancelWallet-{buyer_username}",
            'cus_name': buyer_username,
            'cus_email': "john@example.com",
            'cus_add1': "Address",
            'cus_city': "Dhaka",
...
def redeem_wallet():
        ...
        print(check_card)
        if len(check_card)==0:
            return jsonify({'success':False, 'message':'Please enter a valid card key or key already used'})
        else:
           
            # check_card=check_card[0]
            # print(check_card)
            if check_card[4]=='USED':
                return jsonify({'success':False, 'message':'This card key has been used'})
            else:
                print("denom check:", check_card[3])
                wallet_code_activation_confirm(gift_card,check_card)
            return jsonify({'success':True, 'message':str(check_card[3])+' $ added to account successfully'})
...
def activate_game_key():
        ...
        print("check prod key",check_product_key)
        if len(check_product_key)==0:
            return jsonify({'success':False, 'message':'Please enter a valid game key or key already used or you already own this game'})
        else:
            use_check=check_product_key[4]
            if use_check=='USED':
                return jsonify({'success':False, 'message':'This game key has been used already'})
            else:
                
                game_name=check_product_key[3]
                print(game_name)
...
@app.route('/admin/rotate_user_keys', methods=['POST'])
@login_required('admin')
def admin_rotate_user_keys():
    data = request.json
    username = data.get('username')
    if not username:
    ...
    try:
        ...
        # Force logout if that user is currently logged in (optional: we can clear their session from server-side store)
        # Simpler: tell admin that user must log out and back in.

        return jsonify({'success': True, 'message': f'Keys rotated for {username}. User should log out and log in again.'})
    except Exception as e:
...
def update_password():
    ...
    username = session['username']
    stored_hash = get_stored_password_hash(username)   # from req_auth

    if stored_hash and verify_password(stored_hash, current_password):
        new_hashed = hash_password(new_password)
        update_password_passed_check(new_hashed, username)
        delete_session(session['username'])
        session.clear()
...
@app.route('/update_email_address', methods=['POST'])
@login_required('buyer')
def update_email_address():
    ...
    update_user_info(session['username'],
                     new_email if new_email else None,
                     new_address if new_address else None)
    delete_session(session['username'])
    session.clear()
    return jsonify({'success': True, 'message': 'Profile updated. Please log in again.'})


@app.route('/update_dev_info', methods=['POST'])
@login_required('developer')
def update_dev_info_route():
    data = request.json
    new_email = data.get('new_email')
    new_company = data.get('new_company')
    # Convert empty strings to None to indicate "no change"
    update_dev_info(session['username'],
                    new_email if new_email else None,
...
if __name__ == "__main__":
    connect_db()  # ensure database and tables exist before scheduler runs
    init_session_db()   # ensure session table exists
    scheduler.add_job(id='reset_sales', func=reset_expired_sales, trigger='interval', seconds=90)
    scheduler.start()
    app.run(host='127.0.0.1', port=1097)